...
subject to {
    ...
    forall( e in EDGESET )
	ctWaveWork :
	    	sum ( c in CONFIGSET ) z[c] * c.workcap[e]  <= NWAVELENGTH ;

    forall( e in EDGESET )
	ctWaveProtect :
	    	sum ( c in CONFIGSET ) z[c] * c.protectcap[e]  <= NWAVELENGTH ;
...
execute {
	...
	write("Master Obj : " , cplex.getObjValue() , " Dummy : " , dummysum  , 
            " EXTRA CALL " , EXTRA_CALL[0] , " COLUMNS : " , CONFIGSET.size );

	// copy duals

    for ( var e in EDGESET ) {

        dual_work[ e ] = ctWaveWork[ e ].dual ;
        dual_protect[ e ] = ctWaveProtect[ e ].dual ;
	...
	if ( isModel("FINAL") ) {
        writeln();
		writeln("WITH-INTOBJ         :" , cplex.getObjValue());
		writeln("WITH-RELAXOBJ       :" , RELAXOBJ[ 0 ]  );
		writeln("WITH-GAP            :" , GAP( cplex.getObjValue()  , RELAXOBJ[0] )) ;
        writeln("WITH-USED-WAVE      :" , usedwave );
        writeln("WITH-WORKING-COST   :" , WCOST );
        writeln("WITH-PROTECT-COST   :" , PCOST );
        writeln("WITH-USED-CONFIGS   :" , NUSEDCONFIG );
	} 
    else
    if ( EXTRA_CALL[0] >= 500 ) setNextModel("FINAL");
