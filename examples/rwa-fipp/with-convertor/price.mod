...
dvar int+ TOUCH[ DEMAND ][ DEMAND ] in 0..1 ;
...
minimize cost - sum (e in EDGESET ) ( dual_work[ e ] * x[e] 
                + dual_protect[e] * z[e] ) - sum ( d in DEMAND ) dual_provide[ d ] * p[ d ] ; 
subject to {
    ...
    forall (d1 in DEMAND , d2 in DEMAND , e in EDGESET : d1 != d2 )
        TOUCH[d1][d2] >= ( y[d1][e] + y[d2][e] - 1 );
     

    // if y[d1] touches y[d2] then protect[d1] separate protect[d2]
     forall( d1 in DEMAND , d2 in DEMAND , e in EDGESET : d1 != d2 ){
                protect[d1][e] + protect[d2][e ] <= (2 - TOUCH[d1][d2])  ;
...
execute {
	...
	CONFIGSET.addOnly( newindex , cost.solutionValue, p.solutionValue , 
        working_cost , protect_cost  , x.solutionValue , z.solutionValue  );
