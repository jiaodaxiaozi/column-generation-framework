...
execute STARTSOLVEINT {
    ...
    if ( isModel("RESERVE") ) {
     ...
     if ( isModel("RELAX-RESERVE")  ) setNextModel("PRICE-RESERVE");
}



minimize   sum( l in logicset , e in edgeset ) reserve[ e ][ l ] ; 
       

subject to {

    sum( f in 1..nfailure , l in logicset ) (1-protect[ f ][ l ]) <= MAX_FL[0]; 
...
execute InRelaxProcess {
    ...
    for ( l in logicset )
        insurance[ f ][ l ] = protect[ f ][ l ].solutionValue ;

    for ( e in edgeset )
    for ( l in logicset )
        fixroute[ e ][ l ] = reserve[ e ][ l ].solutionValue ;

	setNextModel("RELAX-RESTORE");
